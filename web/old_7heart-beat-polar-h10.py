...
<head>
  ...
  <title>Polar H10 — Sleep Stage Scoring</title>
  <style>
    :root {
      --bg: #0f172a;
      --panel: #111827;
...
<div class="wrap">
  <header>
    ...
    <h1>Polar H10 → Real-time Sleep Stages</h1>
  </header>

  <!-- Connection Panel -->
  <div class="panel">
    <div class="controls">
      <button id="connectBtn">🔗 Connect H10</button>
      <button id="disconnectBtn" disabled>🔌 Disconnect</button>
      <button id="startRecBtn" disabled>⏺️ Start Recording</button>
      <button id="stopRecBtn" disabled>⏹️ Stop Recording</button>
      ...
      <div class="status" id="status">Checking support…</div>
    </div>
  </div>
  ...
  <div class="panel sleep-stage-panel">
    ...
    <div class="sleep-stage-value" id="sleepStage">—</div>
  ...
  <div class="panel">
    <div class="statbox">
      <div class="stat">
        ...
        <div class="value" id="accVal">— <small>g</small></div>
      </div>
        <div class="stat">
...
// Dynamic backend URL: ?backend=http://IP:5000 overrides, otherwise:
// - if hosted on somehost, use http://somehost:5000
// - else default to http://127.0.0.1:5000
const urlParamBackend = new URLSearchParams(location.search).get('backend');
const BACKEND_URL = urlParamBackend ||
...
const SEND_BATCH_SIZE = 200; // Send samples in batches
const SEND_INTERVAL_MS = 1000; // Send every second
...
let pendingSamples = [];
let backendAvailable = false; // means "server reachable"
let modelReady = false;
let lastBackendSend = 0;
let currentSleepStage = null;
...
async function checkBackend() {
  try {
    ...
    // Reachable if /health returns status ok
    const serverReachable = data && data.status === 'ok';
    backendAvailable = serverReachable;
    modelReady = !!(data && data.model_loaded);

    backendStatus.innerHTML = serverReachable
      ? (modelReady
          ? '<small style="color:var(--good)">✓ Connected (model ready)</small>'
          : '<small style="color:var(--fair)">△ Connected (model not loaded)</small>')
      : '<small style="color:var(--poor)">✗ Offline</small>';

    return serverReachable;
  } catch (e) {
    backendAvailable = false;
    modelReady = false;
    backendStatus.innerHTML = '<small style="color:var(--poor)">✗ Offline</small>';
    return false;
  }
}

async function sendSamplesToBackend() {
  // Only gate on "server reachable"
  ...
  try {
    ...
    // Update progress UI if backend returns collection status
    if (data && data.status && data.status.progress != null) {
      const p = Math.max(0, Math.min(100, data.status.progress));
      progressBar.style.width = `${p.toFixed(0)}%`;
      progressText.textContent = `${p.toFixed(0)}%`;
      if (!data.prediction && data.status.required_duration_s) {
        sleepEpochInfo.textContent = `Collecting data... ${p.toFixed(0)}% (${(data.status.duration_s || 0).toFixed(0)}s / ${data.status.required_duration_s.toFixed(0)}s)`;
      }
    }
    ...
    // Don't flip backendAvailable here; only /health decides online/offline.
...
async function fetchLatestPrediction() {
  ...
  try {
    const resp = await fetch(`${BACKEND_URL}/api/latest`, { method: 'GET' });
    const data = await resp.json();

    if (data && data.status === 'ok' && data.prediction) {
      updateSleepStageDisplay(data.prediction);
    } else {
      // Update progress from collection_status if available
      if (data && data.collection_status && data.collection_status.progress != null) {
        const cs = data.collection_status;
        const p = Math.max(0, Math.min(100, cs.progress));
        progressBar.style.width = `${p.toFixed(0)}%`;
        progressText.textContent = `${p.toFixed(0)}%`;
      }
      if (data && data.message) {
        sleepEpochInfo.textContent = data.message;
      }
    }
  } catch (e) {
...
function updateSleepStageDisplay(pred) {
  ...
  // Update main stage label
  const stage = pred.label || '—';
  sleepStage.textContent = stage;
  sleepStage.className = 'sleep-stage-value ' + stage.toLowerCase();

  // Update epoch info
  const duration = (pred.epoch_end ?? 0) - (pred.epoch_start ?? 0);
  const ts = pred.timestamp ? new Date(pred.timestamp * 1000).toLocaleTimeString() : '';
  sleepEpochInfo.textContent = `Epoch: ${Math.max(0, duration).toFixed(0)}s | Updated: ${ts}`;
  ...
  for (const [label, prob] of Object.entries(probs)) {
    ...
    barDiv.innerHTML = `
      ...
      <div class="bar-bg">
        <div class="bar-fill" style="width: ${(Number(prob) * 100).toFixed(0)}%"></div>
      </div>
    `;
...
async function startRecording(){
  ...
  try{
    ...
    setStatus('Recording… with sleep stages');
  } catch(err){
...
function onPMD(event){
  try {
    ...
    for (let i = 0; i < n; i++){
      ...
      // Add to recording with current sleep stage
      if (recording){
...
function updateUI(){
  if (accBuffer.length > 0){
    ...
    accVal.innerHTML = '— <small>g</small>';
  }
...
async function connect(){
  try{
    ...
    setStatus('Starting accelerometer data stream…');
    ...
    setStatus('Connected. Streaming data + sleep scoring…');
  ...
  checkBackend().then(reachable => {
    if (reachable) {
      setStatus(modelReady
        ? 'Ready. Backend online (model ready). Click Connect H10.'
        : 'Ready. Backend online (loading/absent model). Click Connect H10.');
    } else {
      setStatus('Ready. Backend offline (run server.py). Click Connect H10.');
    }
  });
